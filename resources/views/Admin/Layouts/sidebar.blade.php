...
<aside id="default-sidebar"
    ...
    <div class="h-full px-3 py-4 overflow-y-auto bg-neutral-primary-soft border-e border-default">
        <ul class="space-y-2 font-medium">
            ...
            <li>
                @auth
                    <div class="px-2 py-1.5 text-center border-2 rounded-full border-gray-300 text-body">
                        {{ auth()->user()->name }}
                    </div>
                @endauth
            </li>
            <li>
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit"
                        class="px-4 py-2 w-full text-sm rounded-base bg-red-500 text-white hover:bg-red-600">
                        Logout
                    </button>
                </form>
            </li>
